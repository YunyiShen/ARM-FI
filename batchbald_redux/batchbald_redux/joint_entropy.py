...
class ExactJointEntropy(JointEntropy):
    ...
    def compute_batch(self, log_probs_B_K_C: torch.Tensor, output_entropies_B=None):
        ...
        @toma.execute.chunked(log_probs_B_K_C, initial_step=102400, dimension=0)
        def chunked_joint_entropy(chunked_log_probs_b_K_C: torch.Tensor, start: int, end: int):
            ...
            probs_b_M_C = torch.bmm(
                self.joint_probs_M_K.unsqueeze(0).expand(b, -1, -1),
                chunked_probs_b_K_C
            )
            # 后续操作仍然在 GPU 上
            probs_b_M_C /= K
            output_entropies_B[start:end] = torch.sum(-torch.log(probs_b_M_C) * probs_b_M_C, dim=(1, 2))
